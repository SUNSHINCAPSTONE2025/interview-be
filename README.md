...
> 사용자의 발화와 행동(시선/표정/자세)을 녹화하고, AI 분석으로 즉각적인 정량 피드백을 제공하는 면접 훈련 서비스 (FastAPI)
> 
...
사용자는 답변을 녹화/녹음하고, 서버는 음성/시선/표정/자세를 분석해 **점수와 등급(양호/보통/개선필요 등) + 요약 피드백**을 제공합니다.
...
> 피드백은 즉시 분석하는 형태로 동작하며, 같은 attempt를 다시 조회하면 저장된 결과를 반환하는 방식입니다.
> 
...
- **자세/시선/표정 분석**: `app/services/pose_model.py`, `app/services/face_analysis.py`
