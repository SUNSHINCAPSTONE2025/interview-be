...
자소서 Q&A를 받아서 LLM(GPT-4o-mini)을 사용하여 면접 질문을 생성합니다.
"""
...
def build_prompt_generate_questions(sentences: List[str]) -> str:
    """
    질문 생성을 위한 LLM 프롬프트 생성
...
def ask_llm_for_questions(sentences: List[str]) -> Dict[str, Any]:
    """
    LLM을 호출하여 면접 질문 생성
    ...
    Returns:
        생성된 질문 정보 (key_sentences, questions 포함)

    Raises:
        ValueError: LLM 응답에서 JSON을 파싱할 수 없는 경우
    """
    prompt = build_prompt_generate_questions(sentences)
...
def generate_questions_from_qas(qas: List[Dict[str, str]]) -> Dict[str, Any]:
    """
    자소서 Q&A로부터 면접 질문 생성

    Args:
        qas: 자소서 Q&A 리스트 [{"q": "질문", "a": "답변"}, ...]

    Returns:
        생성된 질문 정보
        {
            "questions": [
                {"text": "질문 텍스트", "type": "job|soft"},
                ...
            ]
        }
    """
    # 1) 답변들을 합쳐서 문장 분할
    joined = "\n".join([qa.get("a", "").strip() for qa in qas if qa.get("a", "").strip()])
    sentences = split_ko(joined)

    # 2) LLM 호출
    result = ask_llm_for_questions(sentences)

    # 3) 필요한 정보만 추출 (text, type만)
    ...
    return {"questions": simplified_questions}
